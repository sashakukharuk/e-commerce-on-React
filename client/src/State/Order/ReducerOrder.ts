...
import {
    ...
    ORDER_SET_ORDER, ORDER_SET_ORDERS, ORDER_SET_PAY_VALUE, ORDER_SET_SUCCESS, ORDER_SET_SUCCESS_STR, ORDER_SET_VALUE
} from "./ActionsOrder";
...
export const orderReducer = (state: OrderStateType, action: ActionsOrderType) => {
    switch (action.type) {
        ...
        case ORDER_SET_PAY_VALUE: {
            return {...state, paymentMethods: action.paymentMethodsL.map(method => {
                    return {
                        value: method.value
                    }
                })}
